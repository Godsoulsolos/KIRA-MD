...
try {
    ...
    console.log("🌍 Connected to the KIRA DB");
} catch {
...
let cc = Config.sessionName.replace(/KIRA;;;/g, "").replace(/BLACK;;;/gi, ""); // DONT REMOVE BLACK;;; so you can connect BLACK md session id too
...
app.listen(port, () => console.log(`Kira Server listening on port http://localhost:${port}!`));
